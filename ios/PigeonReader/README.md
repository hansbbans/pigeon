...
Story rows support native leading/trailing swipes for read and star state, with the same actions in their context menus. Feeds and smart views default to unread stories; Starred defaults to all saved stories so opening one does not hide it. In the reader, a fresh upward swipe that starts at the bottom opens the next article in the selected collection, and a fresh downward swipe that starts at the top opens the previous article. Navigation stops at the collection's ends, and ordinary scrolling does not navigate when it merely reaches a boundary. Website mode keeps normal page scrolling. Selecting a section or feed never opens its first story automatically.
